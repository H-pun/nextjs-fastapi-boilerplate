from sqlalchemy.orm import Session
from api.database import Navigation, Role
from api.core.logger import get_logger

logger = get_logger(__name__)

navigation_admin = [
    Navigation(title="Home", url="/dashboard", icon="home"),
    Navigation(title="User", url="/dashboard/user", icon="user"),
    Navigation(title="Navigation", url="/dashboard/navigation", icon="signpost"),
]

navigation_user = [
    Navigation(title="Home", url="/dashboard", icon="home"),
]

def seed_navigation(db: Session) -> None:
    if db.query(Navigation).count() == 0:
        logger.info("Seeding navigation...")
        
        admin_role = db.query(Role).filter_by(slug="admin").first()
        user_role = db.query(Role).filter_by(slug="user").first()

        def set_order(nav_root: list[Navigation], role: Role) -> None:
            for root_order, root in enumerate(nav_root):
                root.order = root_order
                root.role_id = role.id
                if root.children:
                    for child_order, child in enumerate(root.children):
                        child.order = child_order
                        child.role_id = role.id

        set_order(navigation_admin, admin_role)
        set_order(navigation_user, user_role)

        db.add_all(navigation_admin + navigation_user)
