...
def test_pagination(client: TestClient, url: str, user_token: dict[str, str]):
    ...
    def test_get_all_fail_page(client: TestClient, base_url: str):
        r = client.get(base_url, params={"page": 2, "search": "asd"}, headers=user_token)
        assert r.status_code == 400

    def test_get_all_no_result(client: TestClient, base_url: str):
        r = client.get(base_url, params={"search": "asd"}, headers=user_token)
        assert r.status_code == 200
