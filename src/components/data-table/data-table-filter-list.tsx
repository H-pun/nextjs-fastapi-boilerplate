...
export function DataTableFilterList<TData>({
  ...
  const onFilterRemove = React.useCallback(
    (filterId: string) => {
      ...
      setFiltersAndResetPage(updatedFilters);
  ...
  return (
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        {/* Unused, it is one of a row of buttons and earns only its icon. In
            use, the word and the count are what the row is about. */}
        {filters.length > 0 ? (
